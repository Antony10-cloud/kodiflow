...
export default async function InvoicesPage() {
  ...
  <details><summary>Generate monthly rent</summary><form action={generateMonthlyRent} className="inline-form"><label>Billing month<input name="month" type="month" required /></label><label>Due day<input name="due_day" type="number" min="1" max="28" defaultValue="5" required /></label><label className="checkbox-label"><input name="send_sms" type="checkbox" /> Send each tenant an SMS reminder</label><button>Generate for all tenants</button><small>Existing rent invoices for this month will not be duplicated. SMS is only sent for newly created invoices.</small></form></details></div>
