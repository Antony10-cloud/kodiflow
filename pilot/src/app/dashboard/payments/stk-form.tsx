...
export function StkForm({ references }: { references: string[] }) {
  ...
  return <details className="management-panel"><summary>Send sandbox STK Push</summary><form className="inline-form form-columns" onSubmit={submit}><label>Safaricom phone<input name="phone" placeholder="2547XXXXXXXX" required /></label><label>Amount<input name="amount" type="number" min="1" defaultValue="1" required /></label><label>Tenant reference<select name="reference" required><option value="">Select reference</option>{references.map(ref=><option key={ref}>{ref}</option>)}</select></label><button disabled={busy}>{busy?"Sending…":"Send STK Push"}</button>{message&&<p className="form-message">{message}</p>}</form></details>;
}
