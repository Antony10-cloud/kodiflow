...
export default async function ReceiptsPage() {
  ...
  {!data?.length?<div className="empty-state"><strong>No receipts yet</strong><span>A receipt will be generated after the first matched payment.</span></div>:<div className="table-wrap"><table><thead><tr><th>Receipt</th><th>M-Pesa receipt</th><th>Amount</th><th>Issued</th><th></th></tr></thead><tbody>{data.map((r:any)=><tr key={r.id}><td><code>{r.receipt_number}</code></td><td>{r.payments?.mpesa_receipt}</td><td>{money(Number(r.payments?.amount))}</td><td>{new Date(r.issued_at).toLocaleString("en-KE")}</td><td><a className="table-action" href={`/dashboard/receipts/${r.id}`}>View & print</a></td></tr>)}</tbody></table></div>}</>;
}
